#!/usr/bin/env python3
# ---- slit-aligned kinematics plotter (MCP + Wrist + Elbow Angle + Speed) ----
#
# 2x2 design:
...
#          * mcp_speed, wrist_speed: 1 row (speed), aligned to pass_slit (t=0)
...
#          * mcp_speed, wrist_speed: 1 row (Δspeed)
...
# - Uses pellet_post_x, pellet_post_y, mm_per_px to pellet-center and convert to mm for mcp/wrist (+ speed).
# - Finds each cleaned DLC CSV in CLEANED_DIR using filename_no_ext + CLEANED_SUFFIX
# - Mirrors LEFT camera (cam4) into RIGHT coordinates: x' = IMG_W - x  (and mirrors pellet x too)
# - Windowing:
...
# - Speed:
#     speed = sqrt((dx/dt)^2 + (dy/dt)^2), computed with np.gradient (same length as x/y)
#     by default units are mm/frame; if --fps is provided, speed is mm/s (speed_mm_per_s = speed_mm_per_frame * fps)
...
# FPS (optional; if set, speeds reported in mm/s, otherwise mm/frame)
DEFAULT_FPS = None
...
# Parts
MARKER_PARTS = ["mcp", "wrist"]
SPEED_PARTS = ["mcp_speed", "wrist_speed"]
ANGLE_PARTS = ["elbow_angle"]
PLOT_PARTS_ORDER = ["mcp", "wrist", "mcp_speed", "wrist_speed", "elbow_angle"]

RIGHT_PREFIX = "right_"
LEFT_PREFIX = "left_"

# Joint bodyparts for elbow angle (must match DLC bodypart names)
...
def compute_speed_from_xy(x_mm: np.ndarray, y_mm: np.ndarray, fps: float | None):
    """
    Speed magnitude from x/y time series using np.gradient.
    Returns same-length speed array.
      - If fps is None: units mm/frame
      - Else: units mm/s (multiplies by fps)
    NaNs propagate reasonably: gradient will become NaN where inputs are NaN-heavy.
    """
    ...
    # np.gradient will produce NaNs where inputs are NaN
    dx = np.gradient(x)
    dy = np.gradient(y)

    speed = np.sqrt(dx * dx + dy * dy)
    if fps is not None:
        speed = speed * float(fps)
    return speed


def speed_units_label(fps: float | None) -> str:
    return "mm/s" if fps is not None else "mm/frame"
...
# =========================
def parse_args():
    ap = argparse.ArgumentParser(description="Slit-aligned kinematics plotter with pellet-centering + elbow angle + speed.")
    ...
    ap.add_argument(
        ...
        type=float,
        default=DEFAULT_FPS,
        help="If set, convert speed to mm/s using this fps. If omitted, speed is mm/frame.",
    )
...
def main():
    ...
    if OUT_ALIGNED_CSV is None:
        OUT_ALIGNED_CSV = os.path.join(
            CLEANED_DIR,
            "slit_aligned_mcp_wrist_speed_elbowangle_long_pellet_centered_mm.csv",
        )
    ...
    for _, row in meta.iterrows():
        ...
        pellet_x = safe_float(row["pellet_post_x"])
        pellet_y = safe_float(row["pellet_post_y"])
        mm_per_px = safe_float(row["mm_per_px"])
        if pellet_x is None or pellet_y is None or mm_per_px is None:
            skipped.append((i, fname_base, "NaN pellet_post_x/pellet_post_y/mm_per_px", None))
            continue
        ...
        try:
            ...
            if loaded_debug < DEBUG_FIRST_N:
                ...
                print("fps:", FPS, "| speed units:", speed_units_label(FPS))
                loaded_debug += 1
            ...
            # ---- marker positions (mcp/wrist) + speeds ----
            for part in MARKER_PARTS:
                ...
                # speed record (derived from x_mm/y_mm)
                sp = compute_speed_from_xy(x_mm, y_mm, fps=FPS)
                records.append(pd.DataFrame({
                    "meta_row": i,
                    "trial": fname_base,
                    "subject_key": subject_key,
                    ...
                    "t": t,
                    "speed": sp,
                    "speed_units": speed_units_label(FPS),
                    "hand_lift_off": hand,
                    "pass_slit": slit,
                    "end_frame": end,
                    "cleaned_path": cleaned_path,
    ...
    def plot_four_curves(aligned_df: pd.DataFrame, part: str):
        ...
        # -------- speed --------
        if part in SPEED_PARTS:
            fig, ax = plt.subplots(nrows=1, ncols=1, sharex=True, figsize=(12, 4.5))
            coord = "speed"
            units = speed_units_label(FPS)
            ...
            ax.axvline(0, linestyle="--", linewidth=1, color="black")
            ax.set_title(f"{part.upper()} ({units}) (aligned to pass_slit; t=0) — 4 curves (group × phase)")
    ...
    def plot_delta_curves(aligned_df: pd.DataFrame, part: str):
        ...
        if part in SPEED_PARTS:
            ...
            ax.axhline(0, linestyle=":", linewidth=1, color="black")
            ax.axvline(0, linestyle="--", linewidth=1, color="black")
            ax.set_title(f"{part.upper()} Δ(post - pre) ({units}) (aligned to pass_slit; t=0)")
            ax.set_ylabel(f"ΔSpeed ({units})")
            ax.set_xlabel("t (frames relative to pass_slit)")
            ax.legend()
